...
Nothing about the application changed. The source is identical. What changed is how
much of it you can account for.

---

## Run the agent again

Same agent. Same prompt. One difference: this time it has rails.
...
With a hardened base pinned in its instructions and the Lab 3 policy gate live in the
pipeline, the agent produces something that passes on the first attempt.
...
equivalent, rebuild, and compare. That single measurement will tell you more about
your exposure than a week of reading.

Then store the SBOM as a build artifact, and add one policy gate that fails closed on
signature verification. Everything else can follow from there.
...
Find your facilitator at the Docker booth. Tell them what you build.
